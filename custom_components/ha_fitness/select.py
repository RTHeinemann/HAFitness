...
async def async_setup_entry(
    ...
    entities: list[SelectEntity] = [
        ...
        HAFitnessIntensitySelect(coordinator, entry),
    ]
...
class HAFitnessIntensitySelect(SelectEntity):
    """Shared select entity for activity intensity input."""

    _attr_has_entity_name = True
    _attr_translation_key = "intensity"
    _attr_options = ["low", "moderate", "hard", "very_hard"]

    def __init__(self, coordinator: HAFitnessCoordinator, entry: ConfigEntry) -> None:
        self._coordinator = coordinator
        self._attr_unique_id = f"{entry.entry_id}_intensity"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, entry.entry_id)},
            name=coordinator.display_name,
            manufacturer="HAGym",
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        self._coordinator.set_intensity(option)
